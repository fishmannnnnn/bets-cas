<template>
	<div class="input-wrapper">
		<input
			class="search-input"
			type="search"
			:placeholder="text?text:`Quickly search for a sporting event`"
			name="search" />
	</div>
</template>

<script setup>
	defineProps({
        text: String,
		className: String,
	});
</script>

<style lang="scss" scoped>
	@import "@/styles/global";
	.input-wrapper {
		position: relative;
		z-index: 100;
		&::before {
			content: "";
			position: absolute;
			background: url('data:image/svg+xml,<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M19.7555 18.6065L16.3182 15.2458L16.2376 15.1233C16.0878 14.9742 15.883 14.8902 15.6692 14.8902C15.4554 14.8902 15.2505 14.9742 15.1007 15.1233C12.1795 17.8033 7.67815 17.949 4.58201 15.4637C1.48586 12.9784 0.755668 8.63337 2.87568 5.31017C4.9957 1.98697 9.30807 0.716847 12.9528 2.34214C16.5976 3.96743 18.4438 7.98379 17.267 11.7276C17.1823 11.9981 17.2515 12.2922 17.4487 12.4992C17.6459 12.7062 17.9411 12.7946 18.223 12.7311C18.505 12.6676 18.7309 12.4619 18.8156 12.1914C20.2224 7.74864 18.0977 2.96755 13.8161 0.941058C9.53449 -1.08544 4.38084 0.250824 1.68905 4.08542C-1.00273 7.92001 -0.424821 13.1021 3.04893 16.2795C6.52268 19.4569 11.8498 19.6759 15.5841 16.7949L18.6277 19.7705C18.942 20.0765 19.4502 20.0765 19.7645 19.7705C20.0785 19.4602 20.0785 18.9606 19.7645 18.6503L19.7555 18.6065Z" fill="%23F5AD22" /></svg>');
			width: 20px;
			height: 20px;
			top: 11px;
			left: 8px;
			z-index: 100;
		}
	}
	.search-input::-webkit-search-cancel-button {
		display: none;
	}
	.search-input:focus {
		outline: none;
	}
	.search-input:focus::-webkit-input-placeholder {
		color: transparent;
        opacity: 0;
	}
	.search-input:focus::-moz-placeholder {
		color: transparent;
        opacity: 0;
	}
	.search-input:-moz-placeholder {
		color: transparent;
        opacity: 0;
	}
	.search-input {
		border: none;
		height: 42px;
		width: 100%;
		border-radius: 4px;
		padding: 12.5px 0 12.5px 36px;
		background: $gray-bg-secondary;
		font: $h5-regular;
		color: white;
		&::placeholder {
            caret-color: white;
			color: $gray-text;
		}
	}
</style>
